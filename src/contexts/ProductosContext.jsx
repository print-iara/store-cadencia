...
export function ProductosProvider({ children }) {
    ...
    function obtenerProductos() {
        return (
            new Promise((res, rej) => {
                fetch('https://68335fd5464b499636ff403f.mockapi.io/productos')
                    .then((respuesta) =>
                        respuesta.json()
                    )
                    .then((datos) => {
    ...
    const agregarProducto = (producto) => {
        return (
            new Promise(async (res, rej) => {
                try {
                    const respuesta = await fetch('https://68335fd5464b499636ff403f.mockapi.io/productos', {
                        method: 'POST',
                        headers: {
    ...
    function editarProducto(producto) {
        return (
            new Promise(async (res, rej) => {
                try {
                    const respuesta = await fetch(`https://68335fd5464b499636ff403f.mockapi.io/productos/${producto.id}`, {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json',
                        },
    ...
    const eliminarProducto = (id) => {
        ...
        if (confirmar) {
            return (
                new Promise(async (res, rej) => {
                    try {
                        const respuesta = await fetch(`https://68335fd5464b499636ff403f.mockapi.io/productos//${id}`, {
                            method: 'DELETE',
                        });
